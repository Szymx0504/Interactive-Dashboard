...
function fetchReducer(_s: FetchState, a: FetchAction): FetchState {
    switch (a.type) {
        ...
        case "data":  return { status: "done", sessions: a.sessions, results: a.results };
...
const TooltipContent = ({ active, payload, label, focusedAcronyms, hasFocus, driverMeta }: any) => {
...
function buildTeamGroups(drivers: DriverMeta[]): TeamGroup[] {
    ...
    map.forEach((g) => g.drivers.sort((a, b) => (a.isPrimary ? -1 : 1)));
...
export default function PointsProgressionChart({
    ...
    const visibleSessions = useMemo(() => {
        ...
        const idx = fetchState.sessions.findIndex((s) => s.session_key === selectedSessionKey);
        return idx >= 0 ? fetchState.sessions.slice(0, idx + 1) : fetchState.sessions;
    ...
    const driverMetaMap = useMemo((): Map<string, DriverMeta> => {
        ...
        visibleSessions.forEach((session) => {
            const results: RaceResult[] = fetchState.results[String(session.session_key)] ?? [];
            results.forEach((r) => {
    ...
    const getStyle = useCallback((meta: DriverMeta) => {
    ...
    if (fetchState.status === "error") {
        return <p className="text-red-400 text-sm py-4">Failed to load data: {fetchState.message}</p>;
    }
                                ...
                                <Line
                                    ...
                                    activeDot={
                                        !hasFocus || focusedDrivers.has(meta.driverNumber)
                                            ? { r: 4, fill: meta.colour, stroke: "#fff", strokeWidth: 1.5 }
                                            : false
                                    }
                                    connectNulls
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            backgroundColor: focused ? `${meta.colour}20` : "transparent",
